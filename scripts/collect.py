...
from investor_streak import compute_streak_from_history
...
# 연속 순매수 탐지 설정 (투자자별 최소 연속일)
STREAK_CONFIG = {
    "pension":     {"min_streak": 7,  "lookback": 30},
    "foreigner":   {"min_streak": 5,  "lookback": 20},
    "institution": {"min_streak": 5,  "lookback": 20},
    "individual":  {"min_streak": 5,  "lookback": 20},
}
...
def code_set(payload: dict | None, investor_key: str, market_key: str) -> set[str]:
    ...
    return {str(row.get("ISU_SRT_CD")) for row in rows if row.get("ISU_SRT_CD")}
...
def build_three_day_streak(current_data: dict, recent_trade_dates: list[str]) -> dict:
    result = {}
    previous_1 = load_history_payload(recent_trade_dates[1] if len(recent_trade_dates) > 1 else None)
    previous_2 = load_history_payload(recent_trade_dates[2] if len(recent_trade_dates) > 2 else None)
...
def build_overlap(current_data: dict) -> dict:
    ...
    for market_key, market_name in MARKETS.items():
        ...
        rows_out = [item for item in bucket.values() if len(item["investors"]) >= 2]
        rows_out.sort(key=lambda x: to_int(x.get("total_netbuy_value")), reverse=True)
        result[market_key] = rows_out
    return result
...
def add_all_streaks(payload: dict, warnings: list[str]) -> None:
    """
    연기금: KRX 전체 종목 기반 (pension_streak.py)
    외국인/기관/개인: history TOP20 기반 (investor_streak.py)
    """
    # ── 연기금 (기존 방식 유지) ────────────────────────────
    try:
        pension_result = compute_pension_streak(
            latest_payload=payload,
            history_dir=HISTORY_DIR,
            min_streak=STREAK_CONFIG["pension"]["min_streak"],
            lookback=STREAK_CONFIG["pension"]["lookback"],
        )
        payload["seven_day_streak"] = {
            "pension": {
                "kospi": pension_result.get("kospi", []),
                "kosdaq": pension_result.get("kosdaq", []),
            }
        }
        payload["pension_streak"] = (
            pension_result.get("kospi", []) + pension_result.get("kosdaq", [])
        )
        print("[streak/pension] kospi=%d kosdaq=%d" % (
    ...
    # ── 외국인 / 기관 / 개인 (history TOP20 기반) ──────────
    multi_streak: dict = {}
    for investor_key in ("foreigner", "institution", "individual"):
        cfg = STREAK_CONFIG[investor_key]
        try:
            res = compute_streak_from_history(
                latest_payload=payload,
                history_dir=HISTORY_DIR,
                investor_key=investor_key,
            ...
            multi_streak[investor_key] = res
            print("[streak/%s] kospi=%d kosdaq=%d" % (
                investor_key,
                len(res.get("kospi", [])),
                len(res.get("kosdaq", [])),
            ))
            ...
            traceback.print_exc()
            multi_streak[investor_key] = {"kospi": [], "kosdaq": []}
            warnings.append(f"{investor_key} 연속 순매수 계산 실패: {exc}")

    payload["multi_streak"] = multi_streak


# 하위 호환: backfill_history.py 에서 add_pension_streak 을 직접 호출
def add_pension_streak(payload: dict, warnings: list[str]) -> None:
    add_all_streaks(payload, warnings)
...
def main():
    ...
    candidate_dates = get_recent_trade_dates(
        latest_trade_date=initial_latest_trade_date, count=10,
    )
    ...
    for candidate_trade_date in candidate_dates:
        ...
        recent_trade_dates = get_recent_trade_dates(
            latest_trade_date=candidate_trade_date, count=30,
        )
        current_data = collect_all_investor_data(
            trade_date=candidate_trade_date, warnings=warnings,
        )
        all_records = flatten_records(current_data)
        ...
        if all_records:
            ...
            print(f"[selected] {selected_trade_date} records={len(all_records)}")
            break

        print(f"[skip] {candidate_trade_date} 데이터 0건 — 이전 거래일 확인")

    if not selected_trade_date:
        raise RuntimeError("최근 후보 거래일에서 수급 데이터를 찾지 못했습니다.")
    ...
    add_all_streaks(payload, selected_warnings)
